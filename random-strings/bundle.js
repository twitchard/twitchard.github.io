...
var _updateRule = __webpack_require__(48);
...
var _getDynamicStyles = __webpack_require__(47);
...
var _RegularRule = __webpack_require__(40);

var _RegularRule2 = _interopRequireDefault(_RegularRule);

var _cloneStyle = __webpack_require__(44);
...
var alphanumerics = new Set('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789');
var excludeOptions = [{ set: new Set('0123456789'), display: "Exclude Numbers" }, { set: new Set('ABCDEFGHIJKLMNOPQRSTUVWXYZ'), display: "Exclude Capital Letters" }, { set: new Set('abcdefghijklmnopqrstuvwxyz'), display: "Exclude Lowercase Letters" }, { set: new Set('0Oo'), display: "Exclude 0, O, and o (they look alike)" }, { set: new Set('1I'), display: "Exclude 1 and I (they look alike)" }, { set: new Set('B8'), display: "Exclude B and 8 (they look alike)" }, { set: new Set('S5'), display: "Exclude S and 5 (they look alike)" }, { set: new Set('gq'), display: "Exclude g and q (they look alike)" }, { set: new Set('1l'), display: "Exclude 1 and l (they look alike)" }, { set: new Set('lI'), display: "Exclude l and I (they look alike)" }, { set: new Set('Z2'), display: "Exclude Z and 2 (they look alike)" }, { set: new Set('17'), display: "Exclude 1 and 7 (they look alike)" }, { set: new Set('cCzZ'), display: "Exclude c/C and z/Z (they sound alike)" }, { set: new Set('fFsS'), display: "Exclude f/F and s/S (they sound alike)" }, { set: new Set('mMnN'), display: "Exclude m/M and n/N (they sound alike)" }, { set: new Set('bBpP'), display: "Exclude b/B and p/P (they sound alike)" }, { set: new Set('8aA'), display: "Exclude 8 and a/A (they sound alike)" }];
...
var _class = function () {
    ...
    _createClass(_class, [{
        key: 'onRefresh',
        value: function onRefresh(f) {
            this.refreshSubscribers.push(f);
        }
    }, {
        key: 'onAlphabetLength',
        value: function onAlphabetLength(f) {
            this.alphabetLengthSubscribers.push(f);
        }
    }, {
        key: 'emitAlphabetLength',
        value: function emitAlphabetLength() {
            var n = this.alphabet.size;
            this.alphabetLengthSubscribers.map(function (f) {
                return f(n);
            });
        }
    }, {
        key: 'emitRefresh',
        value: function emitRefresh() {
            this.refreshSubscribers.map(function (f) {
                return f();
            });
        }
    }, {
        ...
        value: function refresh() {
            ...
            var self = this;
            var alphabet = alphanumerics;
            self.labels.selectAll('input').each(function (x) {
                if (this.checked) {
                    alphabet = setMinus(alphabet, x.set);
                }
            });

            var extra = new Set(self.extraLabel.select('input').node().value);
            alphabet = setMinus(alphabet, extra);

            self.alphabet = alphabet;
            self.alphabetList.text(Array.from(this.alphabet.values()).join(""));
        ...
        value: function render() {
            ...
            this.alphabetList = d3.select(this.selector).append('div');

            this.labels = d3.select(this.selector).selectAll('input').data(excludeOptions).enter().append('label').classed('alphabetChooser', true);

            this.labels.append('input').attr('type', 'checkbox').classed('alphabetChooser__checkbox', true).on('change', function () {
                return _this.refresh();
            });

            this.labels.append('span').classed('alphabetChooser__label', true).text(function (x) {
                return x.display;
            });

            this.extraLabel = d3.select(this.selector).append('label');

            this.extraLabel.append('input').on('input', function () {
                return _this.refresh();
            });

            this.extraLabel.append('span').text(function (x) {
                return "Exclude additional characters";
            });

            this.resetButton = d3.select(this.selector).append('div').append('button').on('click', function () {
                return _this.reset();
            }).text('reset');
        ...
        value: function reset() {
            this.alphabet = alphanumerics;
            this.labels.selectAll('input').property('checked', false);

            this.extraLabel.select('input').node().value = '';
...
var _class = function () {
    ...
    _createClass(_class, [{
        ...
        key: "axisLabelSize",
        value: function axisLabelSize(val) {
        ...
        value: function margin(val) {
            ...
            Object.assign(this.settings.margin, { left: left, top: top, right: right, bottom: bottom });
            return this.chart;
        }
    }, {
        key: "attach",
        value: function attach(selector) {
            ...
            this.selector = selector;
            this.svg = d3.select(this.selector).append('svg').attr('width', this.settings.width + this.settings.margin.right + this.settings.margin.left + this.settings.axisLabelSize).attr('height', this.settings.height + this.settings.margin.top + this.settings.margin.bottom + this.settings.axisLabelSize);

            this.line = d3.line().x(function (d) {
                return _this.xScale(d[0]) + _this.settings.margin.left;
            }).y(function (d) {
                return _this.yScale(d[1]) + _this.settings.margin.top;
            });
        }
    }, {
        key: "graph",
        value: function graph() {
            ...
            this._xAxis = this.svg.append('g').attr('class', 'axis');

            this._yAxis = this.svg.append('g').attr('class', 'axis');

            this._xAxisLabel = this.svg.append("text").attr("class", "xaxislabel");

            this._yAxisLabel = this.svg.append("text").attr("class", "yaxislabel");
        }
    }, {
        key: "plot",
        value: function plot() {
            ...
            var n = ~~(this.settings.width / this.settings.step);

            var data = [].concat(_toConsumableArray(Array(n).keys())).map(function (x) {
                return ~~(x * _this2.settings.step * (_this2.settings.xmax / _this2.settings.width));
            }).map(function (x) {
                return [x, _this2.settings.fn(x)];
            });

            this.xScale = d3.scaleLinear().domain([this.settings.xmin, this.settings.xmax]).range([0, this.settings.width]);

            this.yScale = d3.scaleLinear().domain([this.settings.ymin, this.settings.ymax]).range([this.settings.height, 0]);

            this.svg.select('path').attr('d', this.line(data)).attr('transform', "translate(" + this.settings.axisLabelSize + ", 0)");

            var xAxis = d3.axisBottom(this.xScale).ticks(5);

            var yAxis = d3.axisLeft(this.yScale).ticks(5);

            this._xAxis.attr('transform', "translate(" + (this.settings.margin.left + this.settings.axisLabelSize) + ", " + (this.settings.height + this.settings.margin.top) + ")").call(xAxis);

            this._yAxis.attr('transform', "translate(" + (this.settings.margin.left + this.settings.axisLabelSize) + ", " + this.settings.margin.top + ")").call(yAxis);

            this._xAxisLabel.attr("text-anchor", "end").attr("x", this.settings.width).attr("y", this.settings.height + this.settings.margin.top + this.settings.margin.bottom + this.settings.axisLabelSize).text(this.settings.xAxisLabel);

            this._yAxisLabel.attr("text-anchor", "end").attr("dy", ".75em").attr("transform", "rotate(-90)").text(this.settings.yAxisLabel);
        }
    }]);
...
function Slider(elm, _ref) {
    ...
    var type = 'range';
    return Object.assign(elm, { type: type, min: min, max: max, value: initialValue, className: className });
}
...
var _jssNested = __webpack_require__(29);

var _jssNested2 = _interopRequireDefault(_jssNested);

var _jssCamelCase = __webpack_require__(21);

var _jssCamelCase2 = _interopRequireDefault(_jssCamelCase);

var _jssDefaultUnit = __webpack_require__(24);
...
var _jssPropsSort = __webpack_require__(30);

var _jssPropsSort2 = _interopRequireDefault(_jssPropsSort);

var _jssCompose = __webpack_require__(22);

var _jssCompose2 = _interopRequireDefault(_jssCompose);

var _jssExpand = __webpack_require__(25);

var _jssExpand2 = _interopRequireDefault(_jssExpand);

var _jssGlobal = __webpack_require__(28);
...
var _jssPresetDefault = __webpack_require__(15);
...
var _slider4 = __webpack_require__(14);

var _slider5 = _interopRequireDefault(_slider4);

var _alphabet_chooser = __webpack_require__(12);

var _alphabet_chooser2 = _interopRequireDefault(_alphabet_chooser);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var pCollision = function pCollision(alphabetSize) {
    return function (identifierLength) {
        var n = Math.pow(alphabetSize, identifierLength);
        return function (k) {
...
var chart = new _chart2.default('#chart').xmax(10000000).xAxisLabel("Number of identifiers").yAxisLabel("Probability of collision").fn(pCollision(26)(6));

chart.graph();
chart.plot();

var alphabetChooser = new _alphabet_chooser2.default('#alphabetChooser');
alphabetChooser.render();
alphabetChooser.onAlphabetLength(function (x) {
    return alphabetSlider.value = x;
});
alphabetChooser.onAlphabetLength(function (x) {
    return alphabetLabel.value = x;
}
...
var alphabetSlider = void 0;
{
    var elm = document.getElementById('alphabetSlider');
    var options = { max: 64, min: 2, initialValue: 62 };
    alphabetSlider = (0, _slider5.default)(elm, options);
}

var wordLengthSlider = void 0;
{
    var _elm = document.getElementById('wordLengthSlider');
    var _options = { min: 1, max: 12, step: 1, initialValue: 6 };
    wordLengthSlider = (0, _slider5.default)(_elm, _options);
}

var xmaxSlider = void 0;
{
    var _elm2 = document.getElementById('xmaxSlider');
    var _options2 = { min: 1, max: 10, step: 1, initialValue: 6 };
    xmaxSlider = (0, _slider5.default)(_elm2, _options2);
}

var ymaxSlider = void 0;
{
    var _elm3 = document.getElementById('ymaxSlider');
    var _options3 = { min: 1, max: 100, step: 1, initialValue: 100 };
    ymaxSlider = (0, _slider5.default)(_elm3, _options3);
}

var alphabetLabel = document.getElementById('alphabetLabel');
var wordLengthLabel = document.getElementById('wordLengthLabel');
var xmaxLabel = document.getElementById('xmaxLabel');
var ymaxLabel = document.getElementById('ymaxLabel');

function refreshChart() {
    var xmax = Number(xmaxLabel.value);
    var ymax = Number(ymaxLabel.value);
    var alphabetLength = Number(alphabetLabel.value);
    var wordLength = Number(wordLengthLabel.value);

    chart.xmax(xmax);
    chart.ymax(ymax);

    chart.fn(pCollision(alphabetLength)(wordLength));
    chart.plot();
}

alphabetSlider.addEventListener('input', Throttle(100, refreshChart));
alphabetSlider.addEventListener('change', function () {
    return alphabetChooser.reset();
});
...
{
    var label = alphabetLabel;
    var slider = alphabetSlider;
    var updateLabel = function updateLabel() {
        return label.value = slider.value;
    };
    var updateSlider = function updateSlider() {
        return slider.value = label.value;
    };
    updateLabel();
...
{
    var _label = wordLengthLabel;
    var _slider = wordLengthSlider;
    var _updateLabel = function _updateLabel() {
        return _label.value = _slider.value;
    };
    var _updateSlider = function _updateSlider() {
        return _slider.value = _label.value;
    };
    _updateLabel();
    _slider.addEventListener('input', _updateLabel);
    _label.addEventListener('input', _updateSlider);
}

{
    var _label2 = xmaxLabel;
    var _slider2 = xmaxSlider;
    var _updateLabel2 = function _updateLabel2() {
        _label2.value = Math.pow(10, _slider2.value);
    };
    var _updateSlider2 = function _updateSlider2() {
        _slider2.value = Math.log(_label2.value) / Math.log(10);
    };
    _updateLabel2();
    _slider2.addEventListener('input', _updateLabel2);
    _label2.addEventListener('input', _updateSlider2);
}

{
    var _label3 = ymaxLabel;
    var _slider3 = ymaxSlider;
    var _updateLabel3 = function _updateLabel3() {
        return _label3.value = ~~(1000 * (1 - Math.log(101 - _slider3.value) / Math.log(100))) / 1000;
    };
    var _updateSlider3 = function _updateSlider3() {
        return _slider3.value = 101 - Math.pow(100, 1 - _label3.value);
    };
    _updateLabel3();
    _slider3.addEventListener('input', _updateLabel3);
    _label3.addEventListener('input', _updateSlider3);
}
...
var _supportedProperty = __webpack_require__(19);

var _supportedProperty2 = _interopRequireDefault(_supportedProperty);

var _supportedValue = __webpack_require__(20);
...
var _defaultUnits = __webpack_require__(23);
...
var _props = __webpack_require__(26);
...
var _cssVendor = __webpack_require__(18);
...
var _PluginsRegistry = __webpack_require__(33);
...
var _generateClassName = __webpack_require__(46);
...
var _findClassNames = __webpack_require__(45);
...
var _SimpleRule = __webpack_require__(41);

var _SimpleRule2 = _interopRequireDefault(_SimpleRule);

var _KeyframeRule = __webpack_require__(39);
...
var _ViewportRule = __webpack_require__(42);
